...
test: ## [  py] Runs the tests for the project
	...
	@test -d .venv && (. .venv/bin/activate; DOMAIN='https://data.emobon.embrc.eu' REPO_NAME='analysis-results-cluster01-crate'  GENOSCOPE_ID='EMOBON00172' ENA_NR='test_ENAnummer' OBS_ID='VB' ENVPACKAGE_ID='Wa' SAMPLE_MAT_ID='test_sm_id' pytest tests/) || :
	@test -d .venv || echo "no .venv directory found, run 'make init' to initialize the python environment"
